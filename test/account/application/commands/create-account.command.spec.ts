...
describe('CreateAccountCommand', () => {
  ...
  it('throws ConsentNotAcceptedException when consentAccepted is false', async () => {
    await expect(
      sut.execute({ ...validInput, consentAccepted: false }),
    ).rejects.toThrow(ConsentNotAcceptedException);
  });
  ...
  it('creates the user and dispatches account confirmation event', async () => {
    ...
    expect(eventHandler.send).toHaveBeenCalledWith(
      expect.objectContaining({ event: EventTypes.ACCOUNT_CONFIRMATION }),
    );
  });
