...
describe('DomainEntity (via AggregateRoot)', () => {
  describe('constructor', () => {
    ...
    it('defaults createdDate to now when not provided', () => {
      ...
      expect(entity.createdDate.getTime()).toBeGreaterThanOrEqual(
        before.getTime(),
      );
  ...
  describe('update()', () => {
    it('sets updatedDate to a recent timestamp', () => {
      ...
      entity.update();
      const after = new Date();
      expect(entity.updatedDate!.getTime()).toBeGreaterThanOrEqual(
        before.getTime(),
      );
  ...
  describe('delete()', () => {
    it('sets deletedDate to a recent timestamp', () => {
      ...
      entity.delete();
      const after = new Date();
      expect(entity.deletedDate!.getTime()).toBeGreaterThanOrEqual(
        before.getTime(),
      );
