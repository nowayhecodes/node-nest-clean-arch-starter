...
import { DomainValidationException, EmailAlreadyConfirmedException } from '~/shared/domain/exceptions'
...
describe('User', () => {
  describe('User.create()', () => {
    ...
    it('throws DomainValidationException for invalid email', () => {
      expect(() => User.create({ ...validProps, email: 'not-an-email' })).toThrow(DomainValidationException)
    })

    it('throws DomainValidationException when email exceeds 300 chars', () => {
      const longEmail = `${'a'.repeat(295)}@b.com`
      expect(() => User.create({ ...validProps, email: longEmail })).toThrow(DomainValidationException)
    })

    it('throws DomainValidationException for empty firstName', () => {
      expect(() => User.create({ ...validProps, firstName: '' })).toThrow(DomainValidationException)
    })

    it('throws DomainValidationException when firstName exceeds 200 chars', () => {
      expect(() => User.create({ ...validProps, firstName: 'a'.repeat(201) })).toThrow(DomainValidationException)
    })

    it('throws DomainValidationException for empty lastName', () => {
      expect(() => User.create({ ...validProps, lastName: '' })).toThrow(DomainValidationException)
    })

    it('throws DomainValidationException when lastName exceeds 200 chars', () => {
      expect(() => User.create({ ...validProps, lastName: 'a'.repeat(201) })).toThrow(DomainValidationException)
    })

    it('throws DomainValidationException for weak password', () => {
      expect(() => User.create({ ...validProps, password: 'weak' })).toThrow(DomainValidationException)
    })

    it('throws DomainValidationException when password exceeds 100 chars', () => {
      const longPassword = `Secret@1${'a'.repeat(93)}`
      expect(() => User.create({ ...validProps, password: longPassword })).toThrow(DomainValidationException)
    })
  })
