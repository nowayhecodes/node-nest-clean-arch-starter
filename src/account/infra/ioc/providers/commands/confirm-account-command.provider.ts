...
import { ConfirmAccountCommand } from '~/account/application/commands'
...
export class ConfirmAccountCommandProvider {
  static register(): Provider {
    return {
      provide: AccountProviderEnum.COMMANDS.CONFIRM_ACCOUNT,
      useFactory: async (
        securityTokenGenerator: ITokenGenerator,
        userRepository: IUserRepository,
        eventHandler: IEventHandler,
      ): Promise<ConfirmAccountCommand.Contract> => {
        return new ConfirmAccountCommand(securityTokenGenerator, userRepository, eventHandler)
      },
      inject: [
